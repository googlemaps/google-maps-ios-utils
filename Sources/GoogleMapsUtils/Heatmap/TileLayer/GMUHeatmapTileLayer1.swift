...
/// TO-DO: Rename the class to `GMUHeatmapTileLayer` once the linking is done and remove the objective c class.
...
class GMUHeatmapTileLayer1: GMSSyncTileLayer {
    ...
    var tileCreationData: GMUHeatmapTileCreationData1?
    ...
    func setGradient(_ gradient: GMUGradient1) {
    ...
    func setWeightedData(_ weightedData: [GMUWeightedLatLng1]) {
    ...
    func prepare() {
        ...
        let bounds: GQTBounds1 = calculateBounds()
    ...
    func calculateBounds() -> GQTBounds1 {
        var result = GQTBounds1(minX: 0, minY: 0, maxX: 0, maxY: 0)
        ...
        var point: GQTPoint1 = weightedData[0].point()
        result.minX = point.x
        result.maxX = point.x
        result.minY = point.y
        result.maxY = point.y
    ...
    func maxValueForZoom(_ zoom: Int) -> Float? {
        ...
        for dataPoint in weightedData {
            let point: GQTPoint1 = dataPoint.point()
    ...
    func tileFor(x: Double, y: Double, zoom: Double) -> UIImage? {
        ...
        for item in points {
            let point: GQTPoint1 = item.point()
