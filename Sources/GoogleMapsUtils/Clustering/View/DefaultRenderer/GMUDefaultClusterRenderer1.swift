...
/// TO-DO: Rename the class to `GMUDefaultClusterRenderer` once the linking is done and remove the objective c class.
...
///
final class GMUDefaultClusterRenderer1: GMUClusterRenderer1 {
    ...
    private var clusterIconGenerator: GMUClusterIconGenerator1
    /// Current clusters being rendered.
    private var clusters: [GMUCluster1]?
    ...
    private var itemToOldClusterMap: [GMUWrappingDictionaryKey1: GMUCluster1]?
    /// Lookup map from cluster item to a new cluster.
    private var itemToNewClusterMap: [GMUWrappingDictionaryKey1: GMUCluster1]?
    ...
    weak var delegate: GMUClusterRendererDelegate1?
    ...
    ///
    init(mapView: GMSMapView, clusterIconGenerator: GMUClusterIconGenerator1) {
    ...
    ///
    func shouldRenderAsCluster(_ cluster: GMUCluster1, atZoom zoom: Float) -> Bool {
    ...
    ///
    func renderAnimatedClusters(_ clusters: [GMUCluster1]) {
    ...
    func clearMarkersAnimated(_ markers: [GMSMarker]) {
        ...
        for marker in markers {
            ...
            var toCluster: GMUCluster1?
            if let cluster = marker.userData as? GMUCluster1 {
                toCluster = overlappingCluster(for: cluster, itemMap: itemToNewClusterMap)
            } else {
                let key = GMUWrappingDictionaryKey1(object: userData)
    ...
    ///
    func prepareClustersForAnimation(_ newClusters: [GMUCluster1], isZoomingIn: Bool) {
        guard let mapView else { return }
        let zoom: Float = mapView.camera.zoom
        ...
        if isZoomingIn {
            ...
            for cluster in clusters {
                ...
                for clusterItem in cluster.items {
                    let key = GMUWrappingDictionaryKey1(object: clusterItem)
            ...
            for cluster in newClusters {
                ...
                for clusterItem in cluster.items {
                    let key: GMUWrappingDictionaryKey1 = GMUWrappingDictionaryKey1(object: clusterItem)
    ...
    ///
    func addOrUpdateClusters(_ clusters: [GMUCluster1], animated: Bool) {
        guard let mapView else { return }
        ...
        for cluster in clusters {
            ...
            if !shouldShowCluster {
                for item in cluster.items {
                    ...
                    if animated {
                        let key: GMUWrappingDictionaryKey1 = GMUWrappingDictionaryKey1(object: item)
    ...
    ///
    func renderCluster(_ cluster: GMUCluster1, animated: Bool) {
        guard let mapView else { return }
        ...
        if shouldRenderAsCluster(cluster, atZoom: zoom) {
            ...
            for item in cluster.items {
                ...
                if let itemMarker = item as? GMSMarker {
                    ...
                    if shouldAnimate {
                        let key = GMUWrappingDictionaryKey1(object: item)
    ...
    ///
    func visibleClusters(from clusters: [GMUCluster1]) -> [GMUCluster1]? {
        guard let mapView else { return nil }
        var visibleClusters = [GMUCluster1]()
        let zoom = mapView.camera.zoom
        let visibleBounds = GMSCoordinateBounds(region: mapView.projection.visibleRegion())
    ...
    ///
    func overlappingCluster(for cluster: GMUCluster1, itemMap: [GMUWrappingDictionaryKey1: GMUCluster1]?) -> GMUCluster1? {
        var found: GMUCluster1?
        for item in cluster.items {
            let key: GMUWrappingDictionaryKey1 = GMUWrappingDictionaryKey1(object: item)
            if let itemMap, let candidate: GMUCluster1 = itemMap[key] {
                found = candidate
                break
            }
        }
    ...
    func clearMarkers(_ markers: [GMSMarker]) {
        for marker in markers {
            ...
            if marker.userData is GMUCluster1 {
                marker.userData = nil
            }
            marker.userData = nil
