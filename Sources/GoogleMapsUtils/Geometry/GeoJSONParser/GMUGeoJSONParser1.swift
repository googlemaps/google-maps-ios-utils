...
/// TO-DO: Rename the class to `GMUGeoJSONParser` once the linking is done and remove the objective c class.
final class GMUGeoJSONParser1 {
    ...
    private var gmuFeatures: [GMUFeature1] = []
    /// Retrive parsed features
    var features: [GMUFeature1] {
        return gmuFeatures
    }
    ...
    /// Creates `GMUFeature1` from a Dictionary
    ///
    /// - Parameter feature: [String: Any]
    /// - Returns: GMUFeature1?
    func featureFromDictionary(_ feature: [String: Any]) -> GMUFeature1? {
    ...
    /// Creates `GMUGeometry1` from a Dictionary
    ///
    /// - Parameter feature: [String: Any]
    /// - Returns: GMUGeometry1?
    private func extractGeometry(from feature: [String: Any]) -> GMUGeometry1? {
    ...
    /// Creates an array of `GMUFeature1` from the given dictionary
    ///
    /// - Parameter features: [String: Any]
    /// - Returns: [GMUFeature1]?
    func featureCollectionFromDictionary(_ features: [String: Any]) -> [GMUFeature1]? {
    ...
    /// Creates an array of `GMUFeature1` from the features dictionary
    /// Returns an array containing the non-nil `[GMUFeature1]`, line number 295
    ///
    /// - Parameter features: [String: Any]
    /// - Returns: [GMUFeature1]?
    private func parseFeatures(from features: [String: Any]) -> [GMUFeature1]? {
    ...
    /// Creates the `GMUGeometry1` object from the dictionary
    ///
    /// - Parameter dictionary: [String: Any]
    /// - Returns: GMUGeometry1?
    func geometryfromDictionary(_ dictionary: [String: Any]) -> GMUGeometry1? {
    ...
    /// Creates `GMUFeature1` from the geometry JSON
    ///
    /// - Parameter geometryJSON: [String : Any]
    /// - Returns: GMUFeature1?
    func featurefromGeometryDictionary(_ geometryJSON: [String : Any]) -> GMUFeature1? {
    ...
    ///   - geometryArray: [Any]
    /// - Returns: GMUGeometry1
    func geometrywithGeometryType(_ geometryType: String, geometryArray: [Any]) -> GMUGeometry1? {
        switch geometryType {
    ...
    /// Generates `GMUPoint1` from the array of coordinates
    ///
    /// - Parameter coordinate: [Double]
    /// - Returns: GMUPoint1?
    func pointwithCoordinate(_ coordinates: [Double]) -> GMUPoint1? {
        guard let locationFromCoordinateCoordinate = locationFromCoordinate(coordinates) else { return nil }
        return GMUPoint1(coordinate: locationFromCoordinateCoordinate.coordinate)
    }
    
    /// Generates `GMUGeometryCollection1` from the array of multipoint coordinates
    ///
    /// - Parameter coordinates: [[Double]]
    /// - Returns: GMUGeometryCollection1?
    func multiPointwithCoordinates(_ coordinates: [[Double]]) -> GMUGeometryCollection1? {
        let points: [GMUPoint1] = coordinates.compactMap { pointwithCoordinate($0) }
        return points.isEmpty ? nil : GMUGeometryCollection1(geometries: points)
    }

    /// Generates `GMULineString1` from the array of linestring coordinates
    ///
    /// - Parameter coordinates: [[Double]]
    /// - Returns: GMULineString1?
    func lineStringWithCoordinates(_ coordinates: [[Double]]) -> GMULineString1? {
        ...
        if let path: GMSPath = pathFromCoordinateArray(coordinates) {
            ...
            return GMULineString1(path: path)
        }
        return nil
    }

    /// Generates `GMUGeometryCollection1` from the array of multi linestring coordinates
    ///
    /// - Parameter coordinates: [[[Double]]]
    /// - Returns: GMUGeometryCollection1?
    func multiLineStringWithCoordinates(_ coordinates: [[[Double]]]) -> GMUGeometryCollection1 {
        let lineStrings: [GMULineString1] = coordinates.compactMap { lineStringWithCoordinates($0) }
        return GMUGeometryCollection1(geometries: lineStrings)
    }

    /// Generates `GMUPolygon1` from the array of polygon coordinates
    ///
    /// - Parameter coordinates: [[[Double]]]
    /// - Returns: GMUPolygon1?
    func polygonWithCoordinates(_ coordinates: [[[Double]]]) -> GMUPolygon1 {
        var pathArray: [GMSPath] = []
        pathArray = pathArrayFromCoordinateArrays(coordinates)
        return GMUPolygon1(paths: pathArray)
    }

    /// Generates `GMUGeometryCollection1` from the array of polygon coordinates
    ///
    /// - Parameter coordinates: [[[[Double]]]]
    /// - Returns: GMUGeometryCollection1?
    func multiPolygonwithCoordinates(_ coordinates: [[[[Double]]]]) -> GMUGeometryCollection1 {
        let polygons: [GMUPolygon1] = coordinates.map { polygonWithCoordinates($0) }
        return GMUGeometryCollection1(geometries: polygons)
    }
    
    /// Generates `GMUGeometryCollection1` from the array of geometry dictionary
    ///
    /// - Parameter geometries: [[String : Any]]
    /// - Returns: GMUGeometryCollection1?
    func geometryCollectionwithGeometries(_ geometries: [[String : Any]]) -> GMUGeometryCollection1 {
        let elements: [GMUGeometry1] = geometries.compactMap { geometryfromDictionary($0) }
        return GMUGeometryCollection1(geometries: elements)
    }
