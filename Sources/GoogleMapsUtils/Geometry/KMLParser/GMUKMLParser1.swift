...
final class GMUKMLParser1: NSObject, XMLParserDelegate {
    ...
    private var geometry: GMUGeometry1?
    private var geometries: [GMUGeometry1]
    private var title: String?
    private var snippet: String?
    private var inlineStyle: GMUStyle1?
    ...
    /// 
    var placemarksArray: [GMUPlacemark1] {
        return placemarks
    }
    ...
    /// This is called from Unit test class.
    ///
    var styleMapsArray: [GMUStyleMap1] {
        return styleMaps
    }
    ...
    private func handleStyleMap() {
        ...
        let styleMap = GMUStyleMap1(styleMapId: styleID, pairs: pairs)
    ...
    private func parseEndPair() {
        if let key, let styleUrl {
            let pair = GMUPair1(key: key, styleUrl: styleUrl)
            pairs.append(pair)
        }
        key = nil
        styleUrl = nil
    ...
    func parseEndGroundOverlay() {
        ...
        geometry = GMUGroundOverlay1(northEast: northEast, southWest: southWest, zIndex: zIndex, rotation: rotation, href: href)
    ...
    func parseEndPlacemark() {
        ...
        if let geometry {
            let placemark = GMUPlacemark1(geometry: geometry, style: inlineStyle, title: title, snippet: snippet, styleUrl: styleUrl)
    ...
    private func handleMultiGeometryEnd() {
        ...
        geometry = GMUGeometryCollection1(geometries: geometries)
    }
    ...
    private func handlePointElement() {
        if let coordinate: CLLocation = attributes[GMUKMLParserConstants.coordinatesElementName] as? CLLocation {
            geometry = GMUPoint1(coordinate: coordinate.coordinate)
        } else {
    ...
    private func handleLineStringElement() {
        if let path: GMSPath = attributes[GMUKMLParserConstants.coordinatesElementName] as? GMSPath {
            geometry = GMULineString1(path: path)
        } else {
    ...
    private func handlePolygonElement() {
        if let outerBoundaries: GMSPath = attributes[GMUKMLParserConstants.outerBoundariesAttributeName] as? GMSPath {
            ...
            geometry = GMUPolygon1(paths: paths)
        } else {
