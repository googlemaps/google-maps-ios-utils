...
final class GMUGeometryRendererTest: XCTestCase {
    ...
    func testClear() {
         ...
         let renderer = GMUGeometryRenderer(map: mapView, geometries: features)
         renderer.render()
    ...
    func testRenderMarker() {
        ...
        let placemark = GMUPlacemark(geometry: point, style: style, title: titleText, snippet: snippetText, styleUrl: nil)
        let placemarks = [placemark]

        let renderer = GMUGeometryRenderer(map: mapView, geometries: placemarks)
        renderer.render()
    ...
    func testRenderPolyLine() {
        ...
        let modelLineString = GMULineString(path: path)

        let strokeColor = UIColor(red: 1.0, green: 0.0, blue: 0.0, alpha: 1.0)
        let style = self.styleForTest()

        let placemark = GMUPlacemark(geometry: modelLineString, style: style, title: titleText, snippet: nil, styleUrl: nil)
        let placemarks = [placemark]
    ...
    func testGroundOverlay() {
        ...
        let groundOverlay = GMUGroundOverlay(northEast: northEast, southWest: southWest, zIndex: zIndex, rotation: rotation, href: href)
    ...
    func testRenderPolygon() {
        ...
        let modelPolygon = GMUPolygon(paths: paths)

        let strokeColor = UIColor(red: 1.0, green: 0.0, blue: 0.0, alpha: 1.0)
        let fillColor = UIColor(red: 0.5, green: 0.5, blue: 0.5, alpha: 0.5)

        let style = self.styleForTest()
        let placemark = GMUPlacemark(geometry: modelPolygon, style: style, title: titleText, snippet: nil, styleUrl: nil)
        let placemarks = [placemark]
    ...
    func testRenderMultiGeometry() {
        ...
        let firstPoint = GMUPoint(coordinate: position)

        position = CLLocationCoordinate2D(latitude: 12.345, longitude: 23.456)
        let secondPoint = GMUPoint(coordinate: position)

        let geometryCollection = GMUGeometryCollection(geometries: [firstPoint, secondPoint])
        let placemark = GMUPlacemark(geometry: geometryCollection, style: nil, title: nil, snippet: nil, styleUrl: nil)
        let placemarks = [placemark]

        let renderer = GMUGeometryRenderer(map: mapView, geometries: placemarks, styles: nil)
        renderer.render()
    ...
    func testRenderGeometryWithExternalStyle() {
        let position = CLLocationCoordinate2D(latitude: 45.123, longitude: 90.456)
        let point = GMUPoint(coordinate: position)
        let placemark = GMUPlacemark(geometry: point, style: nil, title: titleText, snippet: nil, styleUrl: styleId)
        let placemarks = [placemark]

        let style = self.styleForTest()
        let styles = [style]

        let renderer = GMUGeometryRenderer(map: mapView, geometries: placemarks, styles: styles)
        renderer.render()
    ...
    func testImageFromPathWithURLNotNil() {
        XCTAssertNotNil(GMUGeometryRenderer.image(fromPath: "https://maps.google.com/mapfiles/kml/pal3/icon55.png"))
    }
    ...
    func testGetStyleFromStyleMapsEqualsExpectedStyle() {
        ...
        let pair = GMUPair(key: "normal", styleUrl: "#style")
        let pairArray = [pair]

        let styleMap = GMUStyleMap(styleMapId: "styles", pairs: pairArray)
        let styleMapArray = [styleMap]

        let renderer = GMUGeometryRenderer(map: mapView, geometries: placemarks, styles: styles, styleMaps: styleMapArray)
    ...
    func testGetStyleFromStyleMapsWithEmptyURLAndNilStyleMapObject() {
        ...
        let renderer = GMUGeometryRenderer(map: mapView, geometries: placemarks, styles: styles, styleMaps: nil)
        XCTAssertNil(renderer.getStyle(fromStyleMaps: ""))
    }

    func styleForTest() -> GMUStyle {
        let strokeColor = UIColor(red: 1.0, green: 0.0, blue: 0.0, alpha: 1.0)
        let fillColor = UIColor(red: 0.5, green: 0.5, blue: 0.5, alpha: 0.5)

        return GMUStyle(styleID: styleId,
